    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <p className="text-gray-400 text-sm font-medium">Comissão Total Bruta</p>
          <p className="text-4xl font-display font-bold mt-1 text-white">R$ 45.320</p>
        ...
        >
          ...
          <p className="text-4xl font-display font-bold mt-1 text-white">R$ 12.400</p>
          <p className="mt-4 text-[10px] text-bv-white-ghost uppercase tracking-wider font-bold">3 Pendências ativas</p>
        </motion.div>
        ...
        >
          ...
          <p className="text-gray-400 text-sm font-medium">Previsão Próximo Mês</p>
          <p className="text-4xl font-display font-bold mt-1 text-white">R$ 28.500</p>
          <p className="mt-4 text-[10px] text-bv-white-ghost uppercase tracking-wider font-bold">Baseado no Pipeline</p>
        </motion.div>
      </div>
        ...
        <div className="glass p-8 rounded-3xl">
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-sm text-gray-500">Histórico de rendimentos mensais</p>
            </div>
          </div>
            ...
            {commissions.map((item) => (
              <div key={item.id} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-colors">
                <div className="flex items-center gap-4">
                  <div className={`p-2 rounded-xl bg-white/5 ${item.status === 'Recebido' ? 'text-bv-green' : 'text-gray-400'}`}>
                ...
                <div className="text-right">
                  <p className="font-bold text-white">R$ {item.value.toLocaleString()}</p>
                  <p className={`text-[10px] font-bold uppercase tracking-widest ${
                    item.status === 'Recebido' ? 'text-bv-green' : 'text-bv-white-ghost'
                  }`}>{item.status}</p>
                </div>
              </div>
            ))}
          </div>
          <button className="w-full mt-6 py-3 text-sm text-bv-white-ghost hover:text-white transition-colors">
